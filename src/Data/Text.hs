...
toCaseFold = coerce JSS.toCaseFold
{-# INLINE toCaseFold #-}
...
foldl f a0 (Text t) = JSS.foldl f a0 t
...
foldr f a0 (Text t) = JSS.foldr f a0 t
...
concatMap = coerce JSS.concatMap
...
mapAccumL f z0 (Text a) =
  let (a', t) = JSS.mapAccumL f z0 a
  in (a', Text t)
...
mapAccumR f z0 (Text a) =
  let (a', t) = JSS.mapAccumR f z0 a
  in (a', Text t)
...
replicate = coerce JSS.replicate
...
unfoldr f a = Text $ JSS.unfoldr f a
...
unfoldrN n f a0 = Text $ JSS.unfoldrN n f a0
...
takeWhile = coerce JSS.takeWhile
...
dropWhile = coerce JSS.dropWhile
...
break = coerce JSS.break
-- break p = span (not . p)
{-# INLINE break #-}

-- | /O(n)/ Group characters in a string according to a predicate.
groupBy :: (Char -> Char -> Bool) -> Text -> [Text]
groupBy = coerce JSS.groupBy
-- groupBy p = loop
--   where
...
partition = coerce JSS.partition
-- partition p t = (filter p t, filter (not . p) t)
{-# INLINE partition #-}
...
breakOnAll :: Text              -- ^ @needle@ to search for
           ...
           -> [(Text, Text)]
breakOnAll = coerce JSS.breakOnAll
...
findIndex = coerce JSS.findIndex
...
-- each.
unlines :: [Text] -> Text
unlines = coerce JSS.unlines
-- unlines = concat . L.map (`snoc` '\n')
{-# INLINE unlines #-}

-- | /O(n)/ Joins words using single space characters.
unwords :: [Text] -> Text
unwords = coerce JSS.unwords
...
isPrefixOf = coerce JSS.isPrefixOf
...
isSuffixOf = coerce JSS.isSuffixOf
...
isInfixOf = coerce JSS.isInfixOf
...
stripPrefix = coerce JSS.stripPrefix
...
stripSuffix = coerce JSS.stripSuffix
